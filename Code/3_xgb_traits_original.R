...
set.seed(0)
model.depth6.original <- xgboost(data = dtrain, nrounds = 50, objective = "reg:squarederror",
                        eta = 0.1, max_depth = 6)  # set the depth as 6


### prediction
preds <- predict(model.depth6.original, dtest)
mse <- mean((preds - dat.lst$test.label)^2)
cat("RMSE:", sqrt(mse), "\n")
cat('Variance explained:', 1.0-mse/mean((dat.lst$test.label-mean(dat.lst$test.label))^2), "\n")

# feature importance
importance <- xgb.importance(feature_names = colnames(dat.lst$train.mat), model = model.depth6.original)
importance.plot.depth6.original <- xgb.ggplot.importance(importance) + mytheme + theme(legend.position = 'none')
ggsave('./Data/img/xgb_depth6_original.png', importance.plot.depth6.original, width=10, height=12, dpi=200)

### tune the parameters
set.seed(0)
model.depth10.original <- xgboost(data = dtrain, nrounds = 50, objective = "reg:squarederror",
                         eta = 0.1, max_depth = 10)  # set depth as 10
# prediction
preds <- predict(model.depth10.original, dtest)
mse <- mean((preds - dat.lst$test.label)^2)
cat("RMSE:", sqrt(mse), "\n")
cat('Variance explained:', 1.0-mse/mean((dat.lst$test.label-mean(dat.lst$test.label))^2), "\n")

# model summary
importance <- xgb.importance(feature_names = colnames(dat.lst$train.mat), model = model.depth10.original)
importance.plot.depth10.original <- xgb.ggplot.importance(importance) + mytheme + theme(legend.position = 'none')
ggsave('./Data/img/xgb_depth10_original.png', importance.plot.depth10.original, width=10, height=12, dpi=200)

### save model checkpoints
save(model.depth6.original, file='./Data/Checkpoints/model.depth6.original.RData')
save(model.depth10.original, file='./Data/Checkpoints/model.depth10.original.RData')
